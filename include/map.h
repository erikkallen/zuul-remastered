...
typedef struct Map {
    ...
    int width;
    int height;
    char *backgroundcolor; // Hex-formatted color (#RRGGBB or #AARRGGBB)
                           // (optional)
    char *class;           // The class of the map (since 1.9, optional)
    int compressionlevel;  // The compression level to use for tile layer data
                           // (defaults to -1)
    int hexsidelength; // Length of the side of a hex tile in pixels (hexagonal
                       // maps only)
    ...
    double parallaxoriginx; // X coordinate of the parallax origin in pixels
                            // (since 1.8, default: 0)
    double parallaxoriginy; // Y coordinate of the parallax origin in pixels
                            // (since 1.8, default: 0)
    ...
    char *version; // The JSON format version (previously a number, saved as
                   // string since 1.6)
} Map;
...
uint32_t map_get_tile_id_at_row_col(Map *map, int layer_index, int row,
                                    int col);
