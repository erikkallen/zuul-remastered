...
struct Entity *player_get() { return &player; }
...
void collision_callback(Property *property, void *data) {
    ...
    if (sscanf(property->string_value, "%99[^:]:%d,%d", asset_name, &to_x,
               &to_y) == 3) {
        // Successfully parsed
        ...
        player.x = to_x;
        player.y = to_y;
        // log_debug("Player x: %d y: %d dx: %f dy: %f",player.x, player.y,
        // player.dx, player.dy);
    } else {
        // Parsing failed
        log_error(
            "Failed to parse the input string: %s\nShould be in the format "
            "filename.tmj:x,y where x and y are coordinate to span the "
...
void player_handle(App *app, Map *map, Camera *camera) {
    ...
    if (app->keyboard[SDL_SCANCODE_RIGHT] || app->keyboard[SDL_SCANCODE_LEFT] ||
    ...
    player_move(map);
    // log_debug("Player x: %d y: %d abs %d %d",player.x, player.y,
    // player.x_abs, player.y_abs);
}
...
SDL_Rect player_get_collision_rect(int x, int y) {
    ...
    Tile *player_tile =
        tileset_get_tile_by_id(player.tileset, player.facing, true);
    if (player_tile == NULL) {
    ...
    for (int i = 0; i < player_tile->objectgroup_count; i++) {
        ...
        // log_debug("Object x: %d y: %d width: %d height: %d type: %s",
        // objectgroup[i].x, objectgroup[i].y, objectgroup[i].width,
        // objectgroup[i].height, objectgroup[i].type);
...
bool player_check_map_collision(Map *map, SDL_Rect *player_rect,
    ...
    int player_tile_col_end =
        (player_rect->x + player_rect->w) / map->tilewidth;
    int player_tile_row_end =
        (player_rect->y + player_rect->h) / map->tileheight;
    for (int i = player_tile_col; i <= player_tile_col_end; i++) {
        for (int j = player_tile_row; j <= player_tile_row_end; j++) {
            ...
            if (map_check_tile_collision(map, i, j, player_rect,
                                         intersection)) {
                // log_debug("Collision box x: %d y: %d w: %d h: %d",
                // intersection->x, intersection->y, intersection->w,
                // intersection->h);
                //  Prevent player from moving
                //  Fix player direction during collision
                // log_debug("Collision detected at x: %d [%d %d] y: %d [%d
                // %d]", i,player_tile_col, player_tile_col_end, j,
                // player_tile_row, player_tile_row_end);
                return true;
            }
        }
    }
...
void player_move(Map *map) {
    ...
    SDL_Rect player_rect =
        player_get_collision_rect(player.x + player.dx, player.y);
    ...
    if (player_check_map_collision(map, &player_rect, &distance)) {
        ...
        log_debug("Collision detected at x: %d y: %d w: %d h: %d", player.x,
                  player.y, distance.w, distance.h);
    ...
    if (player_check_map_collision(map, &player_rect, &distance)) {
        ...
        log_debug("Collision detected at x: %d y: %d h: %d w: %d", player.x,
                  player.y, distance.h, distance.w);
    ...
    // Keep player in the center of the camera until the camera hits the edge of
    // the map X Axis movement
...
void player_draw(App *app) {
    ...
    tileset_render_tile(app, player.tileset, player.facing, true,
                        player.x - camera->x, player.y - camera->y,
                        player.move_speed > 0 ? 1 : 0);
}
