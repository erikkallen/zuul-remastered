...
void player_handle(App *app, Map *map, Camera *camera) {
  ...
  player_move(map);
  // log_debug("Player x: %d y: %d abs %d %d",player.x, player.y, player.x_abs,
  // player.y_abs);
}
  ...
  for (int i = player_tile_col; i <= player_tile_col_end; i++) {
    for (int j = player_tile_row; j <= player_tile_row_end; j++) {
      ...
      if (map_check_tile_collision(map, i, j, player_rect, intersection)) {
        // log_debug("Collision box x: %d y: %d w: %d h: %d", intersection->x,
        // intersection->y, intersection->w, intersection->h);
        //  Prevent player from moving
        //  Fix player direction during collision
        // log_debug("Collision detected at x: %d [%d %d] y: %d [%d %d]",
        // i,player_tile_col, player_tile_col_end, j, player_tile_row,
        // player_tile_row_end);
        return true;
      }
    }
