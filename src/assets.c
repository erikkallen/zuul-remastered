...
int asset_init() {
    ...
    //                                      asset_hash, user_compare, NULL,
    //                                      NULL);
    // Load assets.json
    FILE *file = fopen("assets.json", "r");
    if (file == NULL) {
        log_error(
            "Failed to open assets.json the game will not start without it");
        return -1;
    }
    ...
    cJSON_ArrayForEach(asset, assets) {
        ...
        a->filename =
            malloc(strlen(name->valuestring) + strlen(path->valuestring) + 2);
...
char *asset_path(char *filename) {
    log_debug("Looking for asset: %s in total of %d assets", filename,
              asset_count);
