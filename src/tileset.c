    ...
    cJSON_ArrayForEach(j_property, j_properties) {
      ...
      if (cJSON_IsString(j_propertytype)) {
        property->propertytype =
            calloc(strlen(j_propertytype->valuestring) + 1, sizeof(char));
...
Tileset *tileset_load(App *app, const char *filename) {
  ...
  log_info("Loaded tileset name: %s, tile width: %d, tile height: %d, "
           ...
           j_name->valuestring, j_tile_width->valueint, j_tile_height->valueint,
...
static uint32_t tileset_get_current_animation_tileid(Tile *tile) {
  ...
  if (ticks - tile->last_tick > duration) {
    // log_debug("Animating frame %d of %d id ", tile->current_animation_frame,
    // tile->animation_count);
