...
static void tileset_parse_properties(const cJSON *j_tile, Tile *tile) {
    const cJSON *j_properties =
        cJSON_GetObjectItemCaseSensitive(j_tile, "properties");
    ...
    if (cJSON_IsArray(j_properties)) {
        ...
        cJSON_ArrayForEach(j_property, j_properties) {
            ...
            const cJSON *j_name =
                cJSON_GetObjectItemCaseSensitive(j_property, "name");
            if (!cJSON_IsString(j_name)) {
                log_error("Failed to parse tile property name");
                exit(1);
            }
            property->name =
                calloc(strlen(j_name->valuestring) + 1, sizeof(char));
            strcpy(property->name, j_name->valuestring);

            const cJSON *j_type =
                cJSON_GetObjectItemCaseSensitive(j_property, "type");
            if (cJSON_IsString(j_type)) {
                property->type =
                    calloc(strlen(j_type->valuestring) + 1, sizeof(char));
                strcpy(property->type, j_type->valuestring);
            }

            const cJSON *j_propertytype =
                cJSON_GetObjectItemCaseSensitive(j_property, "propertytype");
            if (cJSON_IsString(j_propertytype)) {
                property->propertytype = calloc(
                    strlen(j_propertytype->valuestring) + 1, sizeof(char));
                strcpy(property->propertytype, j_propertytype->valuestring);
            }

            const cJSON *j_value =
                cJSON_GetObjectItemCaseSensitive(j_property, "value");
            if (cJSON_IsString(j_value)) {
                property->string_value =
                    calloc(strlen(j_value->valuestring) + 1, sizeof(char));
...
static void tileset_parse_objectgroup(const cJSON *j_tile, Tile *tile) {
    const cJSON *j_objectgroup =
        cJSON_GetObjectItemCaseSensitive(j_tile, "objectgroup");
    if (cJSON_IsObject(j_objectgroup)) {
        const cJSON *j_objects =
            cJSON_GetObjectItemCaseSensitive(j_objectgroup, "objects");
        ...
        if (cJSON_IsArray(j_objects)) {
            ...
            cJSON_ArrayForEach(j_object, j_objects) {
                ...
                const cJSON *j_x =
                    cJSON_GetObjectItemCaseSensitive(j_object, "x");
                ...
                const cJSON *j_y =
                    cJSON_GetObjectItemCaseSensitive(j_object, "y");
                ...
                const cJSON *j_width =
                    cJSON_GetObjectItemCaseSensitive(j_object, "width");
                if (!cJSON_IsNumber(j_width)) {
                    log_error("Failed to parse object width");
                    exit(1);
                }
                object->width = j_width->valueint;
                const cJSON *j_height =
                    cJSON_GetObjectItemCaseSensitive(j_object, "height");
                ...
                const cJSON *j_type =
                    cJSON_GetObjectItemCaseSensitive(j_object, "type");
                if (cJSON_IsString(j_type)) {
                    object->type =
                        calloc(strlen(j_type->valuestring) + 1, sizeof(char));
                ...
                const cJSON *j_name =
                    cJSON_GetObjectItemCaseSensitive(j_object, "name");
                if (cJSON_IsString(j_name)) {
                    object->name =
                        calloc(strlen(j_name->valuestring) + 1, sizeof(char));
                    strcpy(object->name, j_name->valuestring);
                }
                const cJSON *j_visible =
                    cJSON_GetObjectItemCaseSensitive(j_object, "visible");
...
static void tileset_parse_animation(const cJSON *j_tile, Tile *tile) {
    const cJSON *j_animation =
        cJSON_GetObjectItemCaseSensitive(j_tile, "animation");
    ...
    if (cJSON_IsArray(j_animation)) {
        ...
        cJSON_ArrayForEach(j_frame, j_animation) {
            ...
            const cJSON *j_duration =
                cJSON_GetObjectItemCaseSensitive(j_frame, "duration");
            if (!cJSON_IsNumber(j_duration)) {
                log_error("Failed to parse tile duration");
                exit(1);
            }
            const cJSON *j_tileid =
                cJSON_GetObjectItemCaseSensitive(j_frame, "tileid");
...
Tileset *tileset_load(App *app, const char *filename) {
    ...
    const cJSON *j_tile_width =
        cJSON_GetObjectItemCaseSensitive(tile_json, "tilewidth");
    if (!cJSON_IsNumber(j_tile_width)) {
        log_error("Failed to parse tile width");
        exit(1);
    }
    const cJSON *j_tile_height =
        cJSON_GetObjectItemCaseSensitive(tile_json, "tileheight");
    ...
    const cJSON *j_tilecount =
        cJSON_GetObjectItemCaseSensitive(tile_json, "tilecount");
    if (!cJSON_IsNumber(j_tilecount)) {
        log_error("Failed to parse tilecount");
        exit(1);
    }
    const cJSON *j_columns =
        cJSON_GetObjectItemCaseSensitive(tile_json, "columns");
    if (!cJSON_IsNumber(j_columns)) {
        log_error("Failed to parse columns");
        exit(1);
    }
    const cJSON *j_image_height =
        cJSON_GetObjectItemCaseSensitive(tile_json, "imageheight");
    if (!cJSON_IsNumber(j_image_height)) {
        log_error("Failed to parse imageheight");
        exit(1);
    }
    const cJSON *j_image_width =
        cJSON_GetObjectItemCaseSensitive(tile_json, "imagewidth");
    if (!cJSON_IsNumber(j_image_width)) {
        log_error("Failed to parse imagewidth");
        exit(1);
    }
    const cJSON *j_spacing =
        cJSON_GetObjectItemCaseSensitive(tile_json, "spacing");
    if (!cJSON_IsNumber(j_spacing)) {
        log_error("Failed to parse spacing");
        exit(1);
    }
    const cJSON *j_margin =
        cJSON_GetObjectItemCaseSensitive(tile_json, "margin");
    ...
    if (cJSON_IsArray(j_tiles)) {
        ...
        cJSON_ArrayForEach(j_tile, j_tiles) {
            ...
            const cJSON *j_image =
                cJSON_GetObjectItemCaseSensitive(j_tile, "image");
            if (cJSON_IsString(j_image)) {
                tile->image =
                    calloc(strlen(j_image->valuestring) + 1, sizeof(char));
                strcpy(tile->image, j_image->valuestring);
            }

            const cJSON *j_imageheight =
                cJSON_GetObjectItemCaseSensitive(j_tile, "imageheight");
            if (cJSON_IsNumber(j_imageheight)) {
                tile->imageheight = j_imageheight->valueint;
            }
            const cJSON *j_imagewidth =
                cJSON_GetObjectItemCaseSensitive(j_tile, "imagewidth");
            if (cJSON_IsNumber(j_imagewidth)) {
                tile->imagewidth = j_imagewidth->valueint;
            }
            const cJSON *j_type =
                cJSON_GetObjectItemCaseSensitive(j_tile, "type");
            if (cJSON_IsString(j_type)) {
                tile->type =
                    calloc(strlen(j_type->valuestring) + 1, sizeof(char));
    ...
    log_info("Loaded tileset name: %s, tile width: %d, tile height: %d, "
             "tilecount: %d file: %s size: %d bytes",
             j_name->valuestring, j_tile_width->valueint,
             j_tile_height->valueint, j_tilecount->valueint, filename, fsize);
...
static uint32_t tileset_get_current_animation_tileid(Tile *tile) {
    ...
    if (ticks - tile->last_tick > duration) {
        // log_debug("Animating frame %d of %d id ",
        // tile->current_animation_frame, tile->animation_count);
        tile->current_animation_frame =
            (tile->current_animation_frame + 1) % tile->animation_count;
...
void tileset_render_tile(App *app, Tileset *tileset, int tile_id,
    ...
    // log_debug("Rendering tile %d pos: [%d %d] animated: %d", tileid, x, y,
    // animated);
