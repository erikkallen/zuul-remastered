...
Camera make_camera(App *app, int width, int height) {
    ...
    camera.target = SDL_CreateTexture(
        app->renderer, SDL_PIXELFORMAT_RGBA32, SDL_TEXTUREACCESS_TARGET,
...
void camera_update(Camera *camera, struct Entity *player, int map_width,
                   int map_height) {
