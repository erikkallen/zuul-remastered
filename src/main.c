...
void network_handle(App *app, NetState *state, uint32_t *player_id,
                    struct Entity *player_list, int *player_list_size);
...
int main(int argc, char *argv[]) {
    ...
    while (app.running) {
        if (network) {
            network_handle(&app, &state, &player_id, player_list,
                           &player_list_size);
        }
...
void network_handle(App *app, NetState *state, uint32_t *player_id,
    ...
    if (network_service(&recv_p, state) != 0) {
        ...
        switch (recv_p.type) {
        case ID:
            log_info("Received ID from server *d", recv_p.id);
            break;
        case HOST_CONNECT:
        ...
        case DISCONNECT:
            ...
            for (int i = 0; i < *player_list_size; i++) {
                if (player_list[i].id == recv_p.id) {
                    log_debug("Removed player %u", recv_p.id);
                    player_list[i] = player_list[*player_list_size - 1];
                    player_list_size--;
                    player_list = realloc(player_list, sizeof(struct Entity) *
                                                           (*player_list_size));
                }
