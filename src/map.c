...
static void map_parse_properties(const cJSON *j_object, Object *object) {
    const cJSON *j_properties =
        cJSON_GetObjectItemCaseSensitive(j_object, "properties");
...
static void map_parse_object_layer(const cJSON *j_layer, Layer *layer) {
    const cJSON *j_objects =
        cJSON_GetObjectItemCaseSensitive(j_layer, "objects");
...
int map_load(Map *map, const char *filename) {
    ...
    const cJSON *j_height =
        cJSON_GetObjectItemCaseSensitive(map_json, "height");
    if (!cJSON_IsNumber(j_height)) {
    ...
    const cJSON *j_tile_width =
        cJSON_GetObjectItemCaseSensitive(map_json, "tilewidth");
    if (!cJSON_IsNumber(j_tile_width)) {
        log_error("Failed to parse map tilewidth");
        exit(1);
    }
    const cJSON *j_tile_height =
        cJSON_GetObjectItemCaseSensitive(map_json, "tileheight");
    if (!cJSON_IsNumber(j_tile_height)) {
        log_error("Failed to parse map tileheight");
        exit(1);
    }
    const cJSON *j_layers =
        cJSON_GetObjectItemCaseSensitive(map_json, "layers");
...
void map_draw_layer(App *app, Map *map, Layer *layer) {
    ...
    int32_t end_col = start_col + app->camera->width / map->tilewidth + 1;
    int32_t end_row = start_row + app->camera->height / map->tileheight + 2;
    ...
    // log_debug("Drawing layer w: %d h: %d p: %p, data_p: %p", layer->width,
    // layer->height, layer, layer->data); log_debug("Drawing layer start_col:
    // %d end_col: %d start_row: %d end_row: %d", start_col, end_col, start_row,
    // end_row);
    for (int i = start_col; i < end_col; i++) {
        for (int j = start_row; j < end_row; j++) {
            ...
            tileset_render_tile(app, map->tileset, global_tile_id, false, x, y,
                                true);
        }
    }
}
    ...
    for (int i = 0; i < map->layer_count; i++) {
        ...
        if (map->layers[i].objects != NULL) {
            for (int j = 0; j < map->layers[i].object_count; j++) {
                for (int k = 0; k < map->layers[i].objects[j].property_count;
                     k++) {
                    ...
                    if (map->layers[i].objects[j].properties[k].propertytype !=
                        NULL) {
                        free(map->layers[i]
                                 .objects[j]
                                 .properties[k]
                    ...
                    if (map->layers[i].objects[j].properties[k].string_value !=
                        NULL) {
                        free(map->layers[i]
                                 .objects[j]
                                 .properties[k]
...
uint32_t map_get_tile_id_at_x_y(Map *map, int layer_index, int x, int y) {
    ...
    uint32_t tile_index =
        tile_col + (tile_row * (layer->width / map->tilewidth));
    log_debug("Tile index %d", tile_index);
    return layer->data[tile_index];
}

uint32_t map_get_tile_id_at_row_col(Map *map, int layer_index, int col,
                                    int row) {
...
bool map_check_tile_collision(Map *map, int col, int row, SDL_Rect *bb_rect,
    ...
    SDL_Rect rect = {col * map->tilewidth, row * map->tileheight,
                     map->tilewidth, map->tileheight};
    for (int i = 0; i < tile->property_count; i++) {
        if (strcmp(tile->properties[i].name, "solid") == 0 &&
            tile->properties[i].bool_value == true) {
...
bool map_check_object_collisions(
    Map *map, const char *name, SDL_Rect *player_rect,
    void (*collision_callback)(Property *property, void *data), void *data) {
    for (int i = 0; i < map->layer_count; i++) {
        ...
        // log_debug("Checking objects in layer: %s count: %d",
        // map->layers[i].name, map->layers[i].object_count);
        for (int j = 0; j < layer->object_count; j++) {
            ...
            for (int k = 0; k < object->property_count; k++) {
                ...
                log_debug("Collision detected with object: %s",
                          property->string_value);
