...
void map_draw_layer(App *app, Map *map, Layer *layer) {
  ...
  // layer->height, layer, layer->data); log_debug("Drawing layer start_col: %d
  // end_col: %d start_row: %d end_row: %d", start_col, end_col, start_row,
  // end_row);
...
bool map_check_tile_collision(Map *map, int col, int row, SDL_Rect *bb_rect,
  ...
  SDL_Rect rect = {col * map->tilewidth, row * map->tileheight, map->tilewidth,
                   map->tileheight};
...
bool map_check_object_collisions(
  ...
  for (int i = 0; i < map->layer_count; i++) {
    ...
    // log_debug("Checking objects in layer: %s count: %d", map->layers[i].name,
    // map->layers[i].object_count);
